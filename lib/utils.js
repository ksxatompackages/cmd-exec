...
(function (module) {
	...
	chainFunction.Return = class Return extends Array {
		...
		push(...args) {
			return super.push(...this.filter((val) => val !== chainFunction.NOT_AN_ELEMENT));
		}
	...
	function compareSequence(a, b) {
		if (!isIterable(a) || !isIterable(b) || a.size !== b.size) {
			return false;
		}
	...
	compareSequence.iterable = (a, b) =>
		compareSequence(new Set(a), new Set(b));
	...
	function iterable() {
		return new IterableBase();
	}

	iterable.class = IterableBase;

	class DeepIterable extends IterableBase {
	...
	iterable.deep = (...args) => new DeepIterable(...args);

	iterable.deep.class = DeepIterable;
	...
	deepJoin.many = (sep, ...val) => deepJoin(val, sep);

	function tryCatch(callback, onerror, onsuccess, ...args) {
		...
		try {
			...
			return typeof onerror === 'function' && onerror(error, ...args);
		}
